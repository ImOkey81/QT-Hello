...
static const int kStartColumn = 1;
static const int kLengthColumn = 2;
}
    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->openFileButton, SIGNAL(clicked()), this, SLOT(loadHexFromFile()));
    connect(ui->decodeButton, SIGNAL(clicked()), this, SLOT(decodeHex()));
    connect(ui->addFieldButton, SIGNAL(clicked()), this, SLOT(addFieldRow()));
    connect(ui->removeFieldButton, SIGNAL(clicked()), this, SLOT(removeSelectedFieldRows()));
    connect(ui->saveTemplateButton, SIGNAL(clicked()), this, SLOT(saveTemplate()));
    connect(ui->loadTemplateButton, SIGNAL(clicked()), this, SLOT(loadTemplateSelection()));
    connect(ui->templateList, SIGNAL(currentIndexChanged(int)), this, SLOT(templateSelectionChanged(int)));
...
{
    const QModelIndexList selected = ui->fieldsTable->selectionModel()->selectedRows();
...
QString MainWindow::hexToBitString(const QString &hex) const
{
    ...
    for (int i = 0; i < bytes.size(); ++i) {
        ...
        bits.append(QString("%1").arg(c, 8, 2, QLatin1Char('0')));
    }
    return bits;
}
...
void MainWindow::decodeHex()
{
    ...
    for (int row = 0; row < ui->fieldsTable->rowCount(); ++row) {
        ...
        ui->resultTable->setItem(resultRow, 2, new QTableWidgetItem(okValue ? QString::number(value) : QString("-")));
    }
}
...
void MainWindow::saveTemplate()
{
    ...
    QSqlQuery query;
    query.prepare("INSERT OR IGNORE INTO templates(name) VALUES(:name)");
